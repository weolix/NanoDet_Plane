...
class GhostNet(nn.Module):
    def __init__(
        ...
        self.cfgs = [
            ...
            # 五个参数分别为：
            # k： 卷积核大小
            # t表示exp_size, 表示 Ghost Module 中的扩张卷积层的输出
            #  通道数扩张卷积层可以看作是在输入通道上进行卷积操作的卷
            #  积层，它可以扩张输入通道的维度，从而提高模型的特征表达
            #  能力。在 GhostNet 中，exp_size 的取值通常是输入通道数
            #  的一个较小的倍数，例如 3、6 或 12 等。
            # c：表示 Ghost Module 中的深度可分离卷积层的输出通道数。
            #  深度可分离卷积层是一种轻量级的卷积层，它由深度卷积层和
            #  逐点卷积层组成。在 GhostNet 中，c 的取值通常是一个比较
            #  小的数值，例如 16、24 或 40 等。
            # se_ratio：表示 Squeeze-and-Excitation (SE) 模块中的缩
            #  放因子的通道数比例。SE 模块是一种用于自适应地调整每个通
            #  道的重要性的方法，它可以在不增加网络复杂度的情况下提高模
            #  型的性能。在 GhostNet 中，se_ratio 的取值通常是一个比较
            #  小的数值，例如 0.25 或 0.5 等。
            # s：表示 Ghost Module 中的步幅大小。步幅是卷积核在输入上
            #  滑动的距离，它可以控制输出特征图的大小。在 GhostNet 中，
            #  s 的取值通常是 1 或 2，表示卷积核在输入上滑动的距离为 1 
            #  或 2 个像素。
            # stage1
            [[3, 16, 16, 0, 1]],  # 0
            # stage2
        ...
        if act is not None:
            warnings.warn(
                "Warning! act argument has been deprecated, " 
                "use activation instead!"
            )
            self.activation = act
